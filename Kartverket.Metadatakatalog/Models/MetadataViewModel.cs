        ...
        public string GetCoverageLink()
        {
            ...
            if (!string.IsNullOrEmpty(CoverageUrl) || !string.IsNullOrEmpty(CoverageGridUrl))
            {
                ...
                if ((coverageUrlIndex.HasValue && coverageUrlIndex != -1 ) || (coverageGridUrlIndex.HasValue && coverageGridUrlIndex != -1))
                {
                    ...
                    {
                        if(!string.IsNullOrEmpty(CoverageUrl) && !string.IsNullOrEmpty(CoverageGridUrl))
                            CoverageLink = $"{commonPart}project=geonorge&layers=1002&lat=6768825.17&lon=217236.30&wms=https://wms.geonorge.no/skwms1/wms.geonorge_dekningskart?datasett={layerStr},https://openwms.statkart.no/skwms1/wms.gp_dek_oversikt?datasett={layerStrGrid}&addLayers=geonorgedekningskart&addLayers=geonorgedekningskart,gp_dek_oversikt_wms&type=dek";
